...
interface MultiSelectProps
     ...
     * Optional, defaults to false.
     * @deprecated This prop is not used and will be removed in a future version.
     */
    asChild?: boolean;
...
const warnDuplicates = (duplicates: string[], removed: boolean) => {
    if (duplicates.length > 0) {
        const action = removed ? "automatically removed" : "detected";
        const advice = removed
        ...
        console.warn(`MultiSelect: Duplicate option values ${action}: ${duplicates.join(", ")}. ${advice}`);
    }
};
...
/**
 * Component for rendering a single selected badge
 */
...
const BadgeItem = React.memo<BadgeItemProps>(
    ({
        value,
        option,
        variant,
...
export const MultiSelect = (props: MultiSelectProps & { ref?: React.Ref<MultiSelectRef> }) => {
    const {
        options,
        onValueChange,
        variant,
    ...
    const getAllOptions = React.useCallback((): MultiSelectOption[] => {
        ...
        const allOptions: MultiSelectOption[] = isGroupedOptions(options)
            ? options.flatMap((group) => group.options)
            : options;

        if (!deduplicateOptions) {
            // Detect duplicates in development but don't remove them
            if (process.env.NODE_ENV === "development") {
                ...
                warnDuplicates(duplicates, false);
            }
            return allOptions;
        }
        ...
        warnDuplicates(duplicates, true);
        return uniqueOptions;
    }, [options, deduplicateOptions, isGroupedOptions]);

    const getOptionByValue = React.useCallback(
        (value: string): MultiSelectOption | undefined => {
            const foundOption = getAllOptions().find((opt) => opt.value === value);
            if (!foundOption && process.env.NODE_ENV === "development" && typeof window !== "undefined") {
                window.console?.warn(`MultiSelect: Option with value "${value}" not found in options list`);
            }
            return foundOption;
        },
        [getAllOptions]
    ...
    const { politeMessage, assertiveMessage } = useAccessibilityAnnouncements(
        selectedValues,
        isPopoverOpen,
        searchValue,
        getAllOptions
    ...
    return (
        <>
            <div className="sr-only">
                ...
                <output aria-atomic="true" aria-live="assertive">
                    {assertiveMessage}
                </output>
            </div>
                                    ...
                                    {selectedValues.slice(0, responsiveSettings.maxCount).map((value) => {
                                        ...
                                        return (
                                            <BadgeItem
                                                animation={animation}
                                                animationConfig={animationConfig}
                                                getBadgeAnimationClass={getBadgeAnimationClass}
                                                key={value}
                                                option={option}
